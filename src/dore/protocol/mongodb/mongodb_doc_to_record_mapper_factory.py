...
"""
Build response mapper for mongodb docs
"""
...
def mongodb_doc_to_record_mapper_factory(model: Model) -> Callable[[dict], dict]:
    ...
    def mongodb_doc_to_record_mapper(doc: dict) -> dict:
        record = {}
        for _field_name in doc.keys():
            if _field_name != ID_FIELD_NAME:
                field_value = doc[_field_name]
                _attribute_id = field_name_attribute_id_map[_field_name]
                record[_attribute_id] = field_value

        return record

    return mongodb_doc_to_record_mapper
