...
from dore.protocol.elasticsearch.elasticsearch7.elasticsearch7_create_mapping import elasticsearch7_create_mapping
...
def elasticsearch7_create_model(datastore: ElasticSearch7Datastore, model: Model) -> None:
    client = datastore.client()
    index_name = model.config().properties(ElasticSearchModelPropertiesConfig).index_name()

    LOGGER.info('creating index [%s]', index_name)

    index_mappings = elasticsearch7_create_mapping(model)
