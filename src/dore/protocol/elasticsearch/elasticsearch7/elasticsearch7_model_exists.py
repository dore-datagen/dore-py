...
def elasticsearch7_model_exists(datastore: ElasticSearch7Datastore, model: Model) -> bool:
    client = datastore.client()
    index_name = model.config().properties(ElasticSearchModelPropertiesConfig).index_name()

    LOGGER.info('checking existence for index [%s]', index_name)
